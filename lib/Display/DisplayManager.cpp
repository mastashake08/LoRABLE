...
void DisplayManager::enablePower() {
    pinMode(VEXT_CTRL, OUTPUT);
    digitalWrite(VEXT_CTRL, LOW);  // Active LOW to enable power
}
...
void DisplayManager::showMessage(const String& message, int rssi, float snr) {
    ...
    if (message.length() > 16) {
        ...
        for (int i = 0; i < message.length(); i += 16) {
            String line = message.substring(i, min((int)(i + 16), (int)message.length()));
            display->drawString(0, y, line);
            y += 16;
            if (y > 44) break;  // Don't overflow screen
        }
    } else {
        display->drawStringMaxWidth(0, y, maxWidth, message);
    }
...
void DisplayManager::showStartupScreen() {
    ...
    display->setTextAlignment(TEXT_ALIGN_CENTER);
    display->drawString(64, 10, "LoRABLE by Mastashake");
